...
# Use absolute paths to fix the file-not-found error
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
DATA_DIR = os.path.join(BASE_DIR, '..', '..', 'data', 'analyzed')
# Frontend build directory (updated frontend)
# Try multiple likely locations; prefer React dist if available
_frontend_candidates = [
    os.path.abspath(os.path.join(BASE_DIR, '..', '..', 'frontend', 'build', 'v2')),
    os.path.abspath(os.path.join(BASE_DIR, '..', '..', 'frontend', 'build', 'v1')),
    os.path.abspath(os.path.join(BASE_DIR, '..', '..', 'awakenU-sentiment-analysis', 'frontend_react', 'dist')),
]
FRONTEND_DIR = next((p for p in _frontend_candidates if os.path.isdir(p) and os.path.isfile(os.path.join(p, 'index.html'))), None)
...
# -------------------- Routes --------------------
# Health endpoint (moved from "/" so SPA can serve index.html)
@app.get("/health")
def health():
...
def get_summary():
    ...
    sentiment_counts = {
        ...
        if "neutral" in df["sentiment"].unique() else 0,
    }

    urgency_counts = (
        {k: int(v) for k, v in df["urgency"].value_counts().items()}
        if "urgency" in df.columns else {}
    )
    feedback_type_counts = (
        {k: int(v) for k, v in df["feedback_type"].value_counts().items()}
        if "feedback_type" in df.columns else {}
    )
    ...
    if "platform" in df.columns:
        ...
        for p in sorted(df["platform"].dropna().unique()):
            ...
            platform_comparison.append({
                ...
                if "neutral" in sub["sentiment"].unique() else 0,
            })
    ...
    if "date" in df.columns:
        ...
        if not df.empty:
            ...
            sorted_index = sorted(
                grouped.index,
                key=lambda x: pd.to_datetime(x, format="%b %d %Y", errors="coerce")
            )
...
def get_alerts(
    ...
    try:
        ...
        alerts_data = df.rename(columns={
            "cleaned_body": "details",
            "action_recommended": "action"
        }).to_dict(orient="records")

        sanitized_alerts = [sanitize_record(r) for r in alerts_data]

        return JSONResponse(content=jsonable_encoder(sanitized_alerts))
...
def create_checkout_session(request: Request):
    ...
    try:
        ...
        amount_cents = body.get("amount_cents", 2999)  # Default to Pro plan
        ...
        plan_configs = {
            "basic": {
                ...
                "description": "Export up to 1,000 records with basic analytics"
            },
            "pro": {
                "name": "Pro Export Plan", 
                "description": "Export up to 10,000 records with advanced analytics and custom filters"
            },
            "enterprise": {
                "name": "Enterprise Export Plan",
                "description": "Unlimited exports with all formats, API access, and dedicated support"
            }
        }
...
# -------------------- Static Frontend --------------------
# Mount React build with SPA fallback (serves index.html for unknown routes)
if FRONTEND_DIR:
    app.mount(
        "/",
