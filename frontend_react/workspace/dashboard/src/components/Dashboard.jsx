...
const Dashboard = () => {
  ...
  // State to show/hide payment modal
  const [showPaymentGateway, setShowPaymentGateway] = useState(false);

  // Handle payment success
  const handlePaymentSuccess = () => {
    setShowPaymentGateway(false);
    alert('Payment successful! You can now export your data.');
  };
  ...
  const stats = [
    { title: 'Total Feedback', value: summary.total_feedback.toLocaleString(), icon: MessageSquare, color: 'blue', trend: '+12%' },
    { title: 'Positive Sentiment', value: summary.sentiment_counts.positive.toLocaleString(), icon: TrendingUp, color: 'green', trend: '+8%' },
    { title: 'Negative Sentiment', value: summary.sentiment_counts.negative.toLocaleString(), icon: AlertTriangle, color: 'red', trend: '-3%' },
    { title: 'High Priority', value: highUrgencyCount.toLocaleString(), icon: BarChart3, color: 'orange', trend: '+15%' }
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          onClick={() => setShowPaymentGateway(true)} // 🔹 Added onClick
        >
                ...
                <div key={index} className="flex items-start space-x-3 p-3 bg-white/5 rounded-lg">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center space-x-2 mt-2">
                      <span className="bg-red-500/20 text-red-400 px-2 py-1 rounded text-xs">{alert.urgency || 'High'}</span>
                      <span className="text-gray-400 text-xs">{alert.date || 'Recent'}</span>
                    </div>
                  </div>
                </div>
              )) || (
                <div className="text-gray-400 text-sm text-center py-4">No high urgency alerts found</div>
              )
            )}
          </div>
        </div>
